...
int checkwin() {
	for(int i=1;i<=3;i++) {
		if(square[3*i-2] == square[3*i-1] and square[3*i-1] == square[3*i])
			return 1;
	}
	for(int i=1;i<=3;i++) {
		if(square[i] == square[3+i] and square[3+i] == square[6+i])
			return 1;
	}
...
int main() {
	...
	printTab(2);
	if(csid == -1) {
		printf("FAILED TO CREATE SOCKET AT CLIENT SIDE\n");
		exit(0);
	}
	printf("SUCCESSFULLY CREATED SOCKET AT CLIENT SIDE\n");
	...
	printTab(2);
	if(c == 0) {
		printf("SUCCESSFULLY ESTABLISHED A CONNECTION TO SERVER\n");
	}
	else {
		printf("FAILED TO ESTABLISH A CONNECTION TO SERVER");
	}
	system("sleep 3");
	i = checkwin();
	while(i != 1) {
		...
		printf("\tPLEASE ENTER YOUR CHOICE(X): ");
		scanf("%d",&choice);
		...
		mark='O';
		if(choice<=9 and choice>=1 and (square[choice] == choice + '0'))
			square[choice] = mark;
		clear();
		board();
